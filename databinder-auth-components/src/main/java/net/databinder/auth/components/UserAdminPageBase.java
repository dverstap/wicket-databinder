...
public abstract class UserAdminPageBase<T extends DataUser> extends WebPage {
	...
	public UserAdminPageBase() {
	  add(new DataStyleLink("css"));
		...
		final TextField<String> password = new RSAPasswordTextField("password", new Model<String>(), form) {
			@Override
		...
		form.add(password);
		final TextField<String> passwordConfirm = new RSAPasswordTextField("passwordConfirm", new Model<String>(), form) {
			@Override
      public boolean isRequired() {
				return !isBound();
			}
