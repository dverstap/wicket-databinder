...
public abstract class AuthDataSessionBase<T extends DataUser> extends WebSession implements AuthSession<T> {
   ...
   * @see WebApplication
   */
  public AuthDataSessionBase(final Request request) {
    super(request);
  }
  ...
  protected boolean cookieSignIn() {
    ...
    if (userCookie != null && token != null) {
      ...
      if (potential != null && potential instanceof DataUser) {
        final String correctToken = getApp().getToken(potential);
  ...
  protected void setCookie() {
    ...
    final WebResponse resp = (WebResponse) RequestCycle.get().getResponse();
    ...
    if (rc instanceof CookieRequestCycle) {
      final CookieRequestCycle cookieRc = (CookieRequestCycle) rc;
