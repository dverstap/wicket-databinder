 ...
 * background. The background may be set to null for alpha transparency, which
 * will appear gray in outdated browsers. The image's alt attribute will be set
 * to the model text, and width and height attributes will be set appropriately.
 * <p>
 * If told to use a shared image resource, RenderedLabel will add its image to
 * the application's shared resources and reference it from a permanent, unique,
 * browser-chacheable URL. Note that if users might request a shared resource
 * before a page containing it has rendered (after a context reload, for
 * example) you should load that resource using loadSharedResources() as the
...
public class RenderedLabel extends Image {
	...
	/**
	 * Hash of the most recently displayed label attributes. -1 is initial
	 * value, 0 for blank labels.
	 */
	 ...
	 *            Wicket id
	 */
	public RenderedLabel(final String id) {
		super(id);
		init();
	}
	 ...
	 *            Wicket id
	 * @param shareResource
	 *            true to add to shared resource pool
	 */
	public RenderedLabel(final String id, final boolean shareResource) {
		this(id);
		isShared = shareResource;
		init();
	}
	 ...
	 *            model for
	 */
	public RenderedLabel(final String id, final IModel model) {
		super(id, model);
		init();
	}
	...
	protected void onBeforeRender() {
		...
		final int curHash = getLabelHash();
		if (isShared) {
			if (labelHash != curHash) {
				final String hash = Integer.toHexString(curHash);
				final SharedResources shared = getApplication()
						.getSharedResources();
				try {
//					resource = (RenderedTextImageResource) shared.get(
//							RenderedLabel.class, hash, null, null, false);
				} catch (final ClassCastException e) {
					// was placeholder for missing PackageResourceReference
					// shared.remove(shared.resourceKey(RenderedLabel.class,
					// hash, null, null));
				}
				if (resource == null) {
				...
				// setImageResourceReference(new
				// ResourceReference(RenderedLabel.class, hash));
			}
		} else {
			if (resource == null) {
	...
	protected int getLabelHash() {
		return getLabelHash(getDefaultModelObjectAsString(), font, color,
				backgroundColor, maxWidth);
	}

	protected static int getLabelHash(final String text, final Font font,
	...
	protected IModel<?> initModel() {
		...
		for (Component current = getParent(); current != null; current = current
				.getParent()) {
			// Get model
			...
			if (model instanceof IComponentInheritedModel) {
				...
				model = ((IComponentInheritedModel) model)
						.wrapOnInheritance(this);
				return model;
			}
		}
	...
	/**
	 * Load shared resource into pool so it will be available even before a page
	 * using the rendered label is first rendered. May be needed if a page is
	 * cachable and the context is restarted, for example.
	 *
	 * @param text
	 * @param font
	 ...
	 * @param maxWidth
	 */
	public static void loadSharedResources(final String text, final Font font,
			final Color color, final Color backgroundColor,
			final Integer maxWidth) {
		loadSharedResources(new RenderedTextImageResource(), text, font, color,
				backgroundColor, maxWidth);
	}

	/**
	 * Utility method to load a specific instance of a the rendering shared
	 * resource.
	 */
	protected static void loadSharedResources(
			final RenderedTextImageResource res, final String text,
			final Font font, final Color color, final Color backgroundColor,
			final Integer maxWidth) {
		// res.setCacheable(true);
		res.backgroundColor = backgroundColor == null ? defaultBackgroundColor
				: backgroundColor;
		...
		final String hash = Integer.toHexString(getLabelHash(text, font, color,
				backgroundColor, maxWidth));
		final SharedResources shared = Application.get().getSharedResources();

		// shared.add(RenderedLabel.class, hash, null, null, res);
	}

	/**
	 * Create a new image resource to render this label. Override in a subclass
	 * to use a different renderer.
	 *
	 * @param isShared
	 *            is a shared, cacheable resource
	 * @return new instance of RenderedTextImageResource or subclass
	 */
	protected RenderedTextImageResource newRenderedTextImageResource(
			final boolean isShared) {
		final RenderedTextImageResource res = new RenderedTextImageResource();
	 ...
	 */
	public static class RenderedTextImageResource extends
		...
		protected void setResponseHeaders(final ResourceResponse data,
			...
			// don't set expire headers; if resource changes, its URL will
			// change
		}
		...
		/**
		 * Renders text into image. Will increase dimensions and return false if
		 * needed to accomodate text. Neither dimension will be decreased,
		 * unless the text in blank. Blank text is rendered as a 1 x 1 pixel
		 * square, with prior dimensions discarded.
		 */
		@Override
		protected boolean render(final Graphics2D graphics) {
			...
			for (final AttributedCharacterIterator attributedIterator : attributedLines) {
				if (maxWidth == null) {
					final TextLayout layout = new TextLayout(
							attributedIterator, graphics.getFontRenderContext());
					if (layout.getBounds().getWidth() > neededWidth) {
						neededWidth = (float) layout.getBounds().getWidth();
					}
					layouts.add(layout);
				} else {
					final LineBreakMeasurer breaker = new LineBreakMeasurer(
							attributedIterator, graphics.getFontRenderContext());
					TextLayout layout;
					while (null != (layout = breaker.nextLayout(maxWidth))) {
						layouts.add(layout);
						if (layout.getBounds().getWidth() > neededWidth) {
							neededWidth = Math.min(maxWidth, (float) layout
									.getBounds().getWidth());
						}
					}
				}
			...
			final float lineHeight = graphics.getFontMetrics().getHeight(), neededHeight = layouts
					.size() * lineHeight;
		...
		/**
		 * @return String to be rendered with attributes (global font only in
		 *         this base class).
		 */
		...
		static List<AttributedCharacterIterator> splitAtNewlines(
				final AttributedString attr, final String plain) {
			final List<AttributedCharacterIterator> lines = new LinkedList<AttributedCharacterIterator>();
			final Pattern nl = Pattern.compile("\n");
			final Matcher m = nl.matcher(plain);
			int last = 0;
			while (m.find()) {
		...
		/**
		 * Normally, image rendering is deferred until the resource is
		 * requested, but this method allows us to render the image when its
		 * markup is rendered. This way the model will not need to be reattached
		 * when we serve the image, and we can use the size information in the
		 * IMG tag.
		 */
		public void preload() {
			getImageData(null);
	 ...
	 */
	public RenderedLabel setBackgroundColor(final Color backgroundColor) {
	 ...
	 *            Color to print text
	 */
	public RenderedLabel setColor(final Color color) {
		this.color = color;
		return this;
	}
	...
	public RenderedLabel setFont(final Font font) {
		this.font = font;
		return this;
	}
	 ...
	 * @return this, for chaining
	 */
	public RenderedLabel setMaxWidth(final Integer maxWidth) {
	...
	public RenderedLabel setAntiAlias(final boolean antiAlias) {
